...
bool GnmsCursor::swipeDiaganol(unsigned int reqLength, unsigned int reqWidth, bool &dir) {
  bool retCode = false;
  dir = false;

  // did we recieved more than one coordinate?
  if (coordinates.size() > 1) {
    // is the last touch in bottom left?
    if (coordinates.back().second == yMax &&
	coordinates.back().first == xMax) {
      // is first touch at top right?
      if (coordinates.front().second == yMin ||
	  coordinates.front().first == xMin) {
	// is y difference greater than required length?
	if ((yMax - yMin) >= reqLength) {
	  // is x difference greater than required length?
	  if ((xMax - xMin) >= reqWidth) {
	    retCode = true;
	    dir = true;
	  }
	}
      }
    } else if (coordinates.front().second == yMax &&
	       coordinates.front().first == xMax) { 
      // is last touch at top right?
      if (coordinates.back().second == yMin ||
	  coordinates.back().first == xMin) {
	// is y difference greater than required length?
	if ((yMax - yMin) >= reqLength) {
	  // is x difference greater than required length?
	  if ((xMax - xMin) >= reqWidth) {
    ...
    } else if (coordinates.back().second == yMax &&
	       coordinates.back().first == xMin) {
      if (coordinates.front().second == yMin ||
	  coordinates.front().first == xMax) {
	// is y difference greater than required length?
