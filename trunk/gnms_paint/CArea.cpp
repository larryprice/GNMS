...
bool CArea::onLoad(std::string filename) {
  ...
  tileSet = CSurface::onLoad(string(PAINT_DIR)+string(tileBuf));

  if(tileSet == NULL) {
    fclose(fp);
    cout << PAINT_DIR << tileBuf << " not valid!" << endl;
    return false;
  }
  ...
  for(int x=0; x<areaSize; x++) {
    for(int y=0; y<areaSize; y++) {
      ...
      CMap tempMap;
      if(tempMap.onLoad(string(PAINT_DIR)+string(mapBuf)) == false) {
	fclose(fp);
	cerr << "Failed to load" << PAINT_DIR << mapBuf << endl;
	return false;
      }
