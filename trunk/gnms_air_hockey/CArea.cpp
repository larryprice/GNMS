...
bool CArea::onLoad(std::string filename) {
  ...
  tileSet = CSurface::onLoad(string(HOCKEY_DIR)+string(tileBuf));

  if(tileSet == NULL) {
    fclose(fp);
    cerr << HOCKEY_DIR << tileBuf << " not valid!" << endl;
    return false;
  }
  ...
  for(int x=0; x<areaSize; x++) {
    for(int y=0; y<areaSize; y++) {
      ...
      CMap tempMap;
      if(tempMap.onLoad(string(HOCKEY_DIR)+string(mapBuf)) == false) {
	fclose(fp);
	cerr << HOCKEY_DIR << mapBuf << " not valid!" << endl;
	return false;
      }
